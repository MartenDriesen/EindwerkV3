...
def manage_classes_button(mouse_pos, left_mouse_button, user_id, event, username):
    ...
    global selectedrepo, selected_file, file_doc, answer, member_class_answer  # Add this
    ...
    if panel_open:
        ...
        for idx, class_doc in enumerate(classes):
            ...
            # Draw a cross icon to the right of the class rect
        ...
        for class_doc in classes:
            if selected_class_id == str(class_doc["_id"]):
                ...
                break
        repositories_in_text = font2.render(f"tasks in {selected_class_name}", True, WHITE)
      ...
      # Create Class Button
        ...
        uploadfile_rect.left = addrepo_rect.left
        uploadfile_rect.top = addrepo_rect.bottom + 10  # Adjust this spacing as needed
        ...
        invite_rect.center = (menu_rect.left + 1100, menu_rect.bottom - 80)


        upload_enabled = True if selected_class_role == "member" else False
        open_btn_color = BLUE if file_doc else LIGHT_GRAY
        ...
        # Invite button logic
        invite_enabled = True if selected_class_role == "teach" else False
        invite_btn_color = BLUE if invite_enabled else LIGHT_GRAY
        upload_btn_color = BLUE if invite_enabled else LIGHT_GRAY
        # Invite input field
        ...
        pygame.draw.rect(screen, WHITE, invite_input_box)
        invite_border_color = BLUE if invite_input_active else WHITE
        pygame.draw.rect(screen, invite_border_color, invite_input_box, width=2)
        ...
        pygame.draw.rect(screen, invite_btn_color, invite_rect)
        screen.blit(invite_text, (invite_rect.x + 5, invite_rect.y + 5))
        ...
        # Draw addrepo button
        addrepo_btn_color = BLUE if selected_class_role == "teach" else LIGHT_GRAY
        ...
        repo_table_x = repositories_in_rect.left - 100
...
def upload_feedback(mouse_pos, left_mouse_button, comps, cons):
    ...
    upload_feedbackrect = uploadfeedback_text.get_rect(topleft=(900, 12))
